				   ...
				   argv is the array of those inputs(seperated by spaces).*/
{
	 ...
	 int i;
	 if(argc == 1){ /*argc == 1  means there are no further inputs after testSort2 and
			   testSort2 is considered an arguement itself.
			   Thus, the total count of command line input is 1*/
		 ...
		 mySort(data, nDataItems);//Sorts array using mySort function.
		...
		nDataItems = argc - 1; /*Sets length of data array to the total # of arguments/inputs given by the user.
